...
class JacketPage extends StatefulWidget {
  const JacketPage({Key? key}) : super(key: key);
                ...
                children: [
                  Expanded(
                    child: GridView.builder(
                      itemCount: productController.jackets.length,
...
class JacketCard extends StatelessWidget {
  final int index;

  const JacketCard({Key? key, required this.index}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(
      () {
        if (productController.combo.isEmpty) {
          return CircularProgressIndicator();
        }

        final product = productController.combo[index];
